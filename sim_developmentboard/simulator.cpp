...
// 在全局变量区域添加LED状态变量
std::atomic<int> leds_state[5];  // 仅声明，不要 {1,1,1,1,1}
std::atomic<int> keys[5];        // 仅声明，不要 {1,1,1,1,1}
...
void render(void) {
    ...
    for(int i = 0; i < 5; i++){
        ...
        // 根据状态绘制LED
        if (leds_state[i].load(std::memory_order_relaxed) == 0) {
            // LED亮
...
// 原：std::atomic<int> leds_state[5];
//std::atomic<int> leds_state[5];  // 默认构造（值未定），稍后在 main 中设为 1

// int key_prev_state[5] = {1, 1, 1, 1, 1};
void keyPressed(unsigned char key, int x, int y) {
    switch(key) {
        case 'a':
            keys[0].store(0, std::memory_order_relaxed);   // 或 .store(1, ...)

				restart_triggered = true;
            break;
        case 's':
            keys[1].store(0, std::memory_order_relaxed);   // 或 .store(1, ...)

            break;
        case 'd':
            keys[2].store(0, std::memory_order_relaxed);   // 或 .store(1, ...)

            break;
        case 'f':
            keys[3].store(0, std::memory_order_relaxed);   // 或 .store(1, ...)

            break;
		  case 'g':
            keys[4].store(0, std::memory_order_relaxed);   // 或 .store(1, ...)
...
void keyReleased(unsigned char key, int x, int y) {
    switch(key) {
        case 'a':
            keys[0].store(1, std::memory_order_relaxed);   // 或 .store(1, ...)
            break;
        case 's':
            keys[1].store(1, std::memory_order_relaxed);   // 或 .store(1, ...)
            break;
        case 'd':
            keys[2].store(1, std::memory_order_relaxed);   // 或 .store(1, ...)
            break;
        case 'f':
            keys[3].store(1, std::memory_order_relaxed);   // 或 .store(1, ...)
            break;
		  case 'g':
            keys[4].store(1, std::memory_order_relaxed);   // 或 .store(1, ...)
            break;
    }
}
...
void apply_input() {
    ...
    // }
    display->reset = keys[0].load(std::memory_order_relaxed);
    display->B2    = keys[1].load(std::memory_order_relaxed);
    display->B3    = keys[2].load(std::memory_order_relaxed);
    display->B4    = keys[3].load(std::memory_order_relaxed);
    display->B5    = keys[4].load(std::memory_order_relaxed);

}

void update_leds(){
    // 更新LED状态
    leds_state[0].store(display->led1, std::memory_order_relaxed);
leds_state[1].store(display->led2, std::memory_order_relaxed);
leds_state[2].store(display->led3, std::memory_order_relaxed);
leds_state[3].store(display->led4, std::memory_order_relaxed);
leds_state[4].store(display->led5, std::memory_order_relaxed);
...
void reset() {
	...
	// 重置按键状态
   for (int i = 0; i < 5; i++) {
    keys[i].store(1, std::memory_order_relaxed);
}
...
int main(int argc, char** argv) {
    ...
    for (int i = 0; i < 5; ++i) {
    leds_state[i].store(1, std::memory_order_relaxed);  // LED 初始“灭/未按”
    keys[i].store(1, std::memory_order_relaxed);        // 按键初始“未按”
}
