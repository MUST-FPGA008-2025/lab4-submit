...
module vga_ctrl( // VGA driver
input wire vga_clk , //VGA working clock, 25MHz
input wire sys_rst_n , //Reset signal. Low level is effective
input wire [15:0] pix_data , //Color information from CPU, RGB565 -> 16bits
...
output wire vsync , //Field sync signal
 ...
 //parameter define based on VGA timing diagram: {Standards}
 ...
 //Counter for line sync, the LHS of the Vital Standard Table, Simply follow the timing diagram
 ...
 //Generate line sync signal, based on the cnt_h, we set the hsync
 assign hsync = (cnt_h <= H_SYNC - 1'd1) ? 1'b1 : 1'b0 ;

 //Counter for field sync, the RHS of the Vital Standard Table, also simply follow timing
 always@(posedge vga_clk or negedge sys_rst_n) begin
	 if(sys_rst_n == 1'b0)
		cnt_v <= 10'd0 ;
	 else if((cnt_v == V_TOTAL - 1'd1) && (cnt_h == H_TOTAL-1'd1))// if one frame finished
		cnt_v <= 10'd0 ;
	 else if(cnt_h == H_TOTAL - 1'd1)// if one line finished
		cnt_v <= cnt_v + 1'd1 ;
	 else
		cnt_v <= cnt_v ;// if only "pixel" finished
 end

 //Generate field sync signal,based one cnt_v
 assign vsync = (cnt_v <= V_SYNC - 1'd1) ? 1'b1 : 1'b0 ;

 //Generate image valid signal, the intersection of two bands, line band and field band, 
 // the length of the band is in vga_clk unit, the two counters serve as point demo. activated by vga_clk
